...
export const BRoll: React.FC<{
...
}> = ({ src, dur, pan = "in", fadeIn = 14, fadeOut = 14, dim = 0.32, zoom = 0.12, punch = true }) => {
  ...
  const z = zoom;
  if (pan === "in") scale = 1 + z * p;
  else if (pan === "out") scale = 1 + z * (1 - p);
  else if (pan === "left") { scale = 1 + z; tx = interpolate(p, [0, 1], [z * 60, -z * 60]); }
  else if (pan === "right") { scale = 1 + z; tx = interpolate(p, [0, 1], [-z * 60, z * 60]); }
  else if (pan === "up") { scale = 1 + z; ty = interpolate(p, [0, 1], [z * 60, -z * 60]); }
